...
public class Collision() : BaseIncident<Collision>()
{
    ...
    public override async Task OnTrigger()
    {
        ...
        Utils.ShowNotification("~r~Pull over~s~ with the vehicle and ~y~wait~s~ for the ~f~supervisor~s~ to arrive on-scene.");
        
        foreach (var v in vehiclesInvolved)
        {
            API.ClearEntityLastDamageEntity(Game.PlayerPed.CurrentVehicle.Handle);
